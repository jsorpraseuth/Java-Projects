...
public class IntermediateCharacter implements CharacterIF {
    ...
    private final int startingHealth = 75;
    private final int maxInventorySlots = 3;
    ...
    public void setCurrentWeapon(WeaponIF currentWeapon) throws InvalidWeaponException {
        if(!this.weaponList.contains(currentWeapon)) throw new InvalidWeaponException("The character you tried to set the current weapon: " + currentWeapon.getWeaponName() + ", does not have it in their inventory, you must add the weapon first using setWeapons().");
    ...
    public void setWeapons(List<WeaponIF> weaponList) throws TooManyWeaponsException {
        if (weaponList.size() > maxInventorySlots) throw new TooManyWeaponsException("The max amount of items for a Intermediate Character is: " + maxInventorySlots + " You attempted to add " + weaponList.size() + " which exceeds the maximum.");
    ...
    public void doDamage(CharacterIF victim) {
        System.out.println("Intermediate Character " + this.getCharacterName() + " is doing damage to " + victim.getCharacterName() + " with Intermediate weapon: " +  this.currentWeapon.getWeaponName());
